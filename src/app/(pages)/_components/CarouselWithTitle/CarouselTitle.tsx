...
function CarouselTitle({ title, description, images, backgroundImage, version }: CarouselProps) {
  ...
  return (
    <div className="h-screen bg-white overflow-hidden">
      <div className="h-[40%] max-[490px]:h-[45%] relative overflow-hidden flex items-start max-[490px]:bg-center" style={{ backgroundImage: `url(${illustration})` }}>
        <div className="p-20 w-full max-[490px]:p-0">
          {backgroundImage &&
            (isMobile ? (
              <Image src="https://wtp-assets.oldhat.xyz/products/group-inclusive-tours/Frame_48098208_3_htlsct.png" alt="images" width={1024} height={200} className="absolute" />
            ) : (
              <Image src={backgroundImage} alt="image" width={1200} height={1200} className={version ? 'absolute -top-50 -right-30 rotate-110' : 'absolute top-0 right-0'} />
            ))}
