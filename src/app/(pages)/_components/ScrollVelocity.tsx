...
export const ScrollVelocity: React.FC<ScrollVelocityProps> = ({
  ...
  scrollerStyle,
  textColor = 'black',
  textSize = 'text-4xl md:text-[5rem] md:leading-[5rem]', // Default text size
}) => {
  function VelocityText({
    ...
    return (
      <div className={`${parallaxClassName} relative overflow-hidden`} style={parallaxStyle}>
        <motion.div className={`${scrollerClassName} flex whitespace-nowrap text-center font-sans font-bold tracking-[-0.02em] ${textSize}`} style={{ x, color: textColor, ...scrollerStyle }}>
          {spans}
        </motion.div>
      </div>
    )
    ...
    <section>
      {texts.map((text: string, index: number) => (
        <VelocityText
          ...
          textColor={textColor}
          textSize={textSize} // Pass textSize down to VelocityText
        >
          {text}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        </VelocityText>
      ))}
