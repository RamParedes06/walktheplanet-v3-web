...
function NetflixCarousel() {
  ...
  const current = netflixCarousel[activeIndex] || {
    image: "",
    title: "",
    desc: "",
  };
  ...
  const jumpToSlide = (index: number) => {
    ...
    startTimeRef.current =
      performance.now() - (newProgress / 100) * durationPerSlide * totalSlides;
  };
  ...
  return (
    <div className="relative h-screen flex flex-col justify-center bg-black overflow-hidden">
      ...
      />
      <div
        className="absolute inset-0 top-0 max-[490px]:top-60  bg-gradient-to-b z-1"
        style={{
          background:
            "linear-gradient(180deg, rgba(0, 0, 0, 0) 0.04%, rgba(0, 0, 0, 0.5) 66.87%)",
        }}
      >
        <div className="title-container flex flex-col gap-3 items-start justify-end p-20 max-[490px]:p-10 h-[60vh] max-[490px]:h-[55vh]">
          <p className="title uppercase font-generalSans text-[64px] font-medium max-[490px]:text-[40px]">
            {current.title}
          </p>
          <p className="description font-generalSans text-[20px] max-w-3xl max-[490px]:text-[16px]">
            {current.desc}
          </p>
        </div>

        <div className="flex flex-col max-[490px]:flex-col-reverse px-5 md:-mt-4">
          {/* Buttons */}
          <div className="text-white flex gap-10 justify-end px-20 max-[490px]:px-3  max-[490px]:justify-end max-[490px]:mt-4">
            <span
              onClick={handlePrev}
              className="p-2.5 text-[16px] cursor-pointer"
            >
              Prev
            </span>
            <span
          ...
          <div className="progress mb-4 px-20 max-[490px]:px-5  max-[490px]:mt-0 max-[490px]:mb-2">
        ...
        <div className="carousel h-[30vh] px-20   flex gap-5 items-center justify-center overflow-x-auto max-[490px]:hidden">
          {netflixCarousel.map((item, index) => (
            <div
              key={index}
              className="relative w-[555px] h-[318px] max-[2120px]:w-[655px] max-[2120px]:h-[202px] rounded-xl p-[1px] backdrop-blur-sm border border-white/30 max-[490px]:"
            >
              <div
