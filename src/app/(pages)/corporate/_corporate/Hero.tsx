...
const Hero = ({
  ...
  const getNextIndex = useCallback(
    (current: number) => (current === corpImages.length - 1 ? 0 : current + 1),
    [corpImages.length]
  );

  const getPrevIndex = (current: number) =>
    current === 0 ? corpImages.length - 1 : current - 1;
  ...
  useEffect(() => {
    ...
    if (headerContainerMobileRef.current) {
      if (isMenuVisible) {
        headerContainerMobileRef.current.classList.remove(
          "opacity-0",
          "pointer-events-none"
        );
        headerContainerMobileRef.current.classList.add("opacity-100");
      } else {
        headerContainerMobileRef.current.classList.add(
          "opacity-0",
          "pointer-events-none"
        );
      ...
      <div className="relative w-screen h-[90vh] md:h-screen">
        ...
        >
          ...
          <div className="max-w-[600px] absolute bottom-[15%] left-[5%]">
            <TextReveal>
              <p className="font-satoshi lg:text-[64px] text-[40px] font-semibold max-[490px]:text-[35px]">
                {title}
              </p>
        ...
        <div className=" w-full relative overflow-hidden rounded-xl max-[480px]:rounded-none max-w-full mx-auto flex justify-between">
          <div className="lg:hidden">
            ...
            <div
              ...
              className="fixed z-99 top-[70px]  w-full flex items-center justify-center transition-opacity duration-300"
            >
              {/* Header - only visible when menu is closed */}
              <div
                ref={headerRefMobile}
