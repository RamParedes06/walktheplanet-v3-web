...
export default function ContactForm() {
  const [travelType, setTravelType] = useState<'Leisure Travel' | 'Business Travel'>('Business Travel')
  ...
  const onLeisureSubmit: SubmitHandler<LeisureFormData> = async (data) => {
    try {
      await axios.post(`${process.env.NEXT_PUBLIC_APP_API_BASE_URL}/v2/wtp-contact-us`, { ...data, countryCode: '+63' })
  ...
  return (
    <div className="flex flex-col lg:flex-row w-full h-auto lg:h-screen">
      <section className="w-full lg:w-1/2 bg-[rgb(208,246,255)] flex flex-col justif-start md:justify-end relative overflow-hidden h-[366px] lg:h-full">
        ...
        >
          <Image src={bg} width={1024} height={1024} alt="Background Illustration" className="bg-contain" />
        </div>
        <div className="absolute md:hidden block">
          <Image src={mobileBG} width={1024} height={1024} alt="Background Illustration" className="bg-contain" />
        </div>

        <div className="relative z-10 pr-6 py-15 mt-6 lg:px-20 ml-4 lg:ml-8 mb-6">
          {/* <h1 className="text-[40px] lg:text-[64px] text-[#333] font-normal leading-none"></h1> */}
          <h1 className="text-[40px] lg:text-[64px] text-[#333] font-semibold leading-none mt-2">Let&apos;s plan your dream getaway!</h1>
          <p className="text-base lg:text-[24px] mt-8 lg:mt-8 text-[#333]">Leave the details to us so you can focus on enjoying your journey.</p>
        </div>
      </section>
          ...
          <div className="flex w-full border border-[#14476F] rounded-full overflow-hidden mt-10 mb-5 lg:mb-5">
            <button
              ...
              className={`cursor-pointer w-full py-2 lg:py-3 px-4 font-semibold text-sm transition-all duration-300 ${
                travelType === 'Leisure Travel' ? 'bg-[#14476F] text-white' : 'bg-white text-[#14476F]'
              }`}
            ...
            <button
              type="button"
              className={`cursor-pointer w-full py-2 lg:py-3 px-4 font-semibold text-sm transition-all duration-300 ${
                travelType === 'Business Travel' ? 'bg-[#14476F] text-white' : 'bg-white text-[#14476F]'
              }`}
          ...
          {travelType === 'Leisure Travel' ? (
            <form onSubmit={leisureForm.handleSubmit(onLeisureSubmit)} className="space-y-4 lg:space-y-6 h-150 overflow-auto">
              ...
              <div>
                ...
                <input type="text" placeholder="Input namehere" {...leisureForm.register('firstName')} className="w-full border-b text-black pb-2 focus:outline-none focus:border-gray-600" />
              ...
              <div>
                ...
                <input type="text" placeholder="Input namehere" {...leisureForm.register('middleName')} className="w-full border-b text-black pb-2 focus:outline-none focus:border-gray-600" />
              ...
              <div>
                ...
                <input type="text" placeholder="Input namehere" {...leisureForm.register('lastName')} className="w-full border-b text-black pb-2 focus:outline-none focus:border-gray-600" />
              ...
              {/* Common Fields */}
              <div>
                <label className={`block text-sm font-medium t ${leisureForm.formState.errors.email ? 'text-red-500' : 'text-[#14476F] '} mb-2 `}>
                ...
                <input
                  ...
                  className={`w-full border-b text-black   ${leisureForm.formState.errors.email ? 'text-red-500' : 'text-[#14476F] '} pb-2 focus:outline-none focus:border-gray-600`}
                />
              ...
              <div>
                ...
                <input type="tel" placeholder="Input phone number here" {...leisureForm.register('phoneNumber')} className="w-full border-b text-black pb-2 focus:outline-none focus:border-gray-600" />
              ...
              <div>
                <label className="block text-sm font-medium text-[#14476F] mb-2">Comment</label>
                <input type="text" placeholder="input text here" {...leisureForm.register('comment')} className="w-full border-b text-black pb-2 focus:outline-none focus:border-gray-600" />
              ...
              <button
                ...
                className={`w-full text-white font-medium py-3 rounded-full ${
                  !leisureForm.formState.isValid ? 'bg-[#CCCCCC] opacity-50 pointer-events-none cursor-not-allowed' : 'bg-[#00537F] cursor-pointer'
                } transition-colors`}
              >
                Send to us
              </button>
            ...
            <form onSubmit={businessForm.handleSubmit(onBusinessSubmit)} className="space-y-4 lg:space-y-6 h-150 overflow-auto">
              ...
              {/* Common Fields */}
              <div>
                <label className={`block text-sm font-medium t ${businessForm.formState.errors.email ? 'text-red-500' : 'text-[#14476F] '} mb-2 `}>
                  Email <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  ...
                  className={`w-full border-b border-[#14476F] ${businessForm.formState.errors.email ? 'text-red-500' : 'text-[#14476F] '} text-black pb-2 focus:outline-none focus:border-gray-600`}
                />
              ...
              <div>
                <label className="block text-sm font-medium text-[#14476F] mb-2">Comment</label>
                <input placeholder="input text here" {...businessForm.register('comment')} className="w-full border-b border-[#14476F] text-black focus:outline-none focus:border-gray-600 pb-2" />
              ...
              <button
                type="submit"
                className={`w-full text-white font-medium py-3 rounded-full ${
                  !businessForm.formState.isValid ? 'bg-[#CCCCCC] opacity-50 pointer-events-none cursor-not-allowed' : 'bg-[#00537F] cursor-pointer'
                } transition-colors`}
              >
                Send to us
              </button>
        ...
        <Modal
          iconSvg={
            <svg width="35" height="35" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M26.6663 8L11.9997 22.6667L5.33301 16" stroke="#8CC152" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          }
          iconBorder="#D8EACC"
        ...
        <Modal
          iconSvg={
            <svg width="35" height="35" viewBox="0 0 50 50" fill="none" xmlns="http://www.w3.org/2000/svg">
              <line x1="12" y1="12" x2="38" y2="38" stroke="#F27474" strokeWidth="6" strokeLinecap="round" />
              <line x1="38" y1="12" x2="12" y2="38" stroke="#F27474" strokeWidth="6" strokeLinecap="round" />
            </svg>
          }
          iconBorder="#F27474"
